...
#                 FROM edu_course_exercise_qa WHERE exercise_id = %s'%(courseId)
#             qa = getDict(qaSql)
#             for i in qa:
...
def getExercise():
    ...
    sql = 'SELECT t1.id, t1.question_type, t1.course_id, t1.content, t1.source, t1.easy_error, t2.id AS question_id, \
        t2.question, t2.cando, t2.type, t2.total_score, t2.answer, t3.id AS comment_id, t3.`comment` \
            FROM (SELECT * FROM edu_course_exercise WHERE question_type = %r AND checked = 1 AND state != 1 LIMIT 1) AS t1 \
            LEFT JOIN edu_course_exercise_qa AS t2 on t2.exercise_id = t1.id \
            LEFT JOIN edu_course_exercise_comment AS t3 ON t3.course_exercise_qa_id = t2.id;'%(type)
    result = getDict(sql)
    if result:
        qa['id'] = result[0]['id']
...
def getOptions():
    qaId = request_parse(request).get("qaId")
    data = {
        'code': 0,
        'data': [],
        'errMessage': ''
    }
    sql = "SELECT val, text FROM edu_course_exercise_option WHERE qa_id = %r"%(qaId)
    print(sql)
    try:
        data['data'] = sqlOperate.op_select(sql)
...
def goback_case():
    try:
        sql =  "UPDATE edu_course_exercise set state=0 "+\
                f"where state = 1 and checked = 0 and update_time < date_sub(now(), interval 5 MINUTE);"
